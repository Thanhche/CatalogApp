...
# return a list of item names of a specific category
...
def catalogMenu():
    ...
    categories = session.query(Category)
    items = session.query(MenuItem).filter_by(kind="new").all()
    return render_template('pub_catalog.html', categories=categories,
                                                items=items)
...
def editMenuItem(category_name, item_name):
    ...
    category = session.query(Category).filter_by(name=category_name).one()
    editedItem = session.query(MenuItem).filter_by(name=item_name,
...
def newMenuItem(category_name):
    ...
    if request.method=='POST':
        ...
        #url_response=urllib2.Request(request.form['image'])
        #try:
            #image=request.form['image']
            #urllib2.urlopen(url_response)
        #except:
